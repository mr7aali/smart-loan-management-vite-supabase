...
export const calculateTotalInterest = (
  ...
  return roundMoney(principal * (rate / 100) * months);
};

export const calculateMonthlyFlatInterest = (
  principal: number,
  rate: number,
): number => {
...
export const calculateTotalPayable = (
  ...
  return roundMoney(
    principal + calculateTotalInterest(principal, rate, months),
  );
};
...
export const normalizeBorrower = (
  borrower: Partial<Borrower> | null | undefined,
): Borrower => {
  return {
    id: borrower?.id || "",
...
export const normalizeLoan = (loan: Partial<Loan> | null | undefined): Loan => {
  const normalizedBorrower = loan?.borrowers
    ? normalizeBorrower(loan.borrowers)
    : undefined;
...
export const normalizeRepayment = (
  repayment: Partial<Repayment> | null | undefined,
): Repayment => {
  const normalizedLoan = repayment?.loans
    ? normalizeLoan(repayment.loans)
    : undefined;
...
export const getLoansByBorrower = (
  loans: Loan[],
  borrowerId: string,
): Loan[] => {
...
export const getRepaymentsByLoan = (
  repayments: Repayment[],
  loanId: string,
): Repayment[] => {
